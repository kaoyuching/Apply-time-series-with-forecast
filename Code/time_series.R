...
x <- read.csv("C:\\Users\\User\\Desktop\\all\\data_mining/time.csv",header = T) 
x1 <- x[1:15,]
...
####basic forecasts
#forecast from mean
mf <- meanf(y,h = 6,level = c(90,95),fan=FALSE,lambda=NULL)
plot(mf)
...
##Check for identifying under lying patterns:
#Stationary & non-stationary, seasonality, trend.
...
#adf test
adf1 <- adf.test(diff_data)
adf1  #p-value=0.0496 >> stationary

auto.arima(diff(y))
forecast(auto.arima(diff(y)),h=5)
plot(forecast(auto.arima(diff(y)),h=5))